...
All 18 visual graphs were imported/exported through ComfyUI and checked against synthetic API examples built by the panel, including dynamic inputs, links and widget values. Writer seeds are fixed in examples for reproducibility. The owner completed real-generation acceptance for the released workflows and reported no obvious issue; existing device acceptance covers FL2VA 9/9 and Ref2VA 9/9 INT8 generation, Qwen Ref2VA role replacement, a simple Ollama character case and representative BF16 generation. Raw language understanding and complex character recognition remain model limitations. A full BF16 matrix is not claimed. The owner intentionally waived a fresh clean-install GPU smoke for this Public Beta; it will be repeated when the installer or dependency contract changes.
