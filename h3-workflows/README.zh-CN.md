...
18 个图形工作流均通过 ComfyUI 原生导入/导出，并与面板构建的脱敏 API 示例核对动态输入、连线和控件值；示例将提示词种子固定以便复现。用户已完成发布工作流的真实生成验收且未发现明显问题；既有真机验收覆盖 FL2VA 9/9、Ref2VA 9/9 INT8 生成、Qwen Ref2VA 角色替换、Ollama 简单角色，以及代表性 BF16 生成。Raw 自然语言理解和复杂角色识别仍属于模型能力边界，不宣称完整 BF16 矩阵。本次 Public Beta 由项目负责人明确豁免全新安装后的 GPU smoke；安装器或依赖契约发生变化时再单独复验。
