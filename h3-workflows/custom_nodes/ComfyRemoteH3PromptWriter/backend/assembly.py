...
def _validate_reference_tags(brief: str, manifest: dict[str, Any], mode: str) -> None:
    ...
    if missing:
        ...
        raise AssemblyError(
            "REFERENCE_NOT_FOUND",
            f"{tag} doesn't exist. Add the reference or remove the tag from the Creative Brief.",
            {"reference": tag},
        )
...
def _final_contract(mode: str, task_text: str) -> str:
    ...
    return (
        ...
        "invented ending. For every visual role explicitly assigned to a Picture, describe concrete visible attributes "
        "from that Picture in the permitted domains; never replace them with vague placeholders such as 'specific style', "
        "'as shown', or 'the referenced clothing'. If a detail cannot be identified reliably, omit that detail instead of "
        "inventing it. "
        "Music requested without an uploaded audio asset belongs only in non_diegetic_music and must "
...
def assemble_request(body: dict[str, Any]) -> dict[str, Any]:
    ...
    role_lock = str(body.get("reference_role_lock") or "").strip()
    role_lock_block = (
        f"\n\nReference role lock (strict allowlist; user brief remains authoritative):\n{role_lock}"
        if role_lock else ""
    )
    user_content = (
        f"Mode: {mode}\n"
