...
@pytest.mark.asyncio
async def test_v042_generic_false_failure_repairs_from_disk_when_history_is_gone(
    panel_client_v043
):
    created = await _create_job(panel_client_v043, "history evicted recovery")
    job_id = created["id"]
    await panel_client_v043.app["db"].update_job(
        job_id,
        status="failed",
    ...
    files = panel_client_v043.app["files"]
    raw_name = f"{files.storage_key(job_id)}_00001_.mp4"
    raw_path = files.output_root / raw_name
    raw_path.write_bytes(b"\x00\x00\x00\x18ftypisom" + b"\x00" * 32)
...
@pytest.mark.asyncio
async def test_disk_recovery_refuses_ambiguous_multiple_mp4_outputs(panel_client_v043):
    created = await _create_job(panel_client_v043, "ambiguous recovery")
    job_id = created["id"]
    await panel_client_v043.app["db"].update_job(
        job_id,
        status="failed",
    ...
    files = panel_client_v043.app["files"]
    storage_key = files.storage_key(job_id)
    for suffix in ("00001_", "00002_"):
        (files.output_root / f"{storage_key}_{suffix}.mp4").write_bytes(
