...
### D-11（P1）vivo/Edge 移动端表单与导航不稳定

2026-08-25 在 vivo X300、Edge、中国广电 5G、Tailscale 已连接环境中发现：分辨率 range 在纵向滚动时容易误触，偶发唤起输入法；时长 select 操作不便；页面存在左右滑动空间；底部 fixed 三页导航受移动浏览器动态工具栏影响，视觉位置不稳定。

核实结果：分辨率确为原生 `input[type=range]`，应用没有主动唤起键盘；基础 378 px Chromium 视口未复现溢出，问题可能由 Edge/vivo 原生控件、动态视口和实际长文件名共同触发。CSS 中长文件名 flex 子项缺少完整收缩约束，页面也未设置横向裁切。底部导航明确使用 `position: fixed`。

优化方向：分辨率改为离散步进/预设按钮并减少滚动误触；时长改为 5–15 秒离散滑杆，并提供“跟随参考视频 1（取整并限制范围）”快捷项；导航移至顶部 sticky 分段栏；补 `min-width: 0`、长文本断行和 `overflow-x: clip`。3–4 秒输出与节点标注的约 5 秒起始训练范围冲突，先作为实验项，不默认开放。

### D-12（P1）任务反馈和媒体呈现信息不足

- 当前数值进度只来自 ComfyUI sampling `progress` 事件；构建、加载、解码、合成和保存阶段只有文字，因此总进度条长期显示 0。
- 任务卡无缩略图是上一轮为避免移动网络重复加载 metadata 而主动采用的设计，不是网络故障。
- 下载名直接沿用 ComfyUI 的 `video_00001_.mp4`。
- 载入原参数后只显示“沿用原任务图片”，没有真实缩略图或足够明确的保留素材卡片。
- Ref2VA 约 16.5 MB 参考视频上传完成后，数据库任务在 8 ms 内开始执行；面板没有记录或显示上传进度，无法区分“正在上传”与“提交慢”。该次真实生成约 10 分 50 秒。

优化方向：为工作流节点定义阶段权重，把构建/采样/解码/合成/保存映射到统一 0–100%；任务卡进入视口后一次性懒加载首帧缩略图；下载名改为预设、时间和任务短 ID；重试草稿展示可辨识的保留素材缩略图/卡片；使用可观测上传进度并显示已上传大小、总大小和耗时。

### D-13（P1/P2）Ref2VA 素材管理不完整

已确认：参考图片、视频和音频使用一个 `multiple` 文件输入；预览没有单项删除/替换按钮，只能整组选取。参考视频只有文件名，不能预览。音频 input 未设置 `capture`，应用没有调用麦克风；Android/Edge 文件选择器因音频 MIME 类型提供录音入口，容易误解为必须授权麦克风。当前“参考图比例”只取第一张参考图，不支持明确选择参考图 1 或参考视频 1 的画幅。

优先修复：用前端受控文件数组支持每项预览、删除和替换；参考视频用本地 Object URL 预览；音频选择器优先按扩展名过滤并明确标注“选择已有音频文件，无需麦克风”；画幅增加“参考图 1”和“参考视频 1”两个来源。图片、视频和音频裁剪需要新增浏览器编辑器或安全 FFmpeg 派生文件链，复杂度和回归面较大，按用户决定暂缓，不作为当前发布阻断项。
...
- [ ] **T-011 修复移动端控件和导航稳定性。** 分辨率防误触、时长滑杆、顶部 sticky 导航、横向溢出约束，并在 vivo X300 Edge 复验。
- [ ] **T-012 实现统一阶段进度和上传反馈。** 工作流阶段折算为总进度；大素材提交显示上传字节、百分比和耗时。
- [ ] **T-013 改善任务媒体呈现。** 视口内懒加载视频首帧缩略图，下载使用可辨识名称，重试草稿明确展示保留素材。
- [ ] **T-014 完善 Ref2VA 单项素材管理。** 图片/视频/音频逐项预览、删除和替换；音频明确文件选择；画幅来源区分参考图 1/参考视频 1。
- [ ] **T-015 暂缓素材裁剪。** 图片画幅裁剪、视频区间裁剪和音频区间裁剪不进入当前版本；如以后恢复，单独设计 FFmpeg/浏览器处理、安全边界和失败回滚。
