...
更新日期：2026-08-25
适用版本：`0.1.0` 当前工作区  
文档状态：已完成代码与本机静态/自动化复核，并补充首轮手机/Tailscale 实机结果；真实 ComfyUI/GPU 安全关闭复验和移动网络 Direct 问题仍待完成。

## 1. 结论摘要

项目主体结构完整，现有自动化检查全部通过，但当前不应发布。阻断发布的核心问题是：移动网络无法建立 Tailscale Direct 连接、64 位 seed 经过浏览器 JSON 后丢精度、wheel 缺少六套工作流、声明的最低依赖不能运行现有代码，以及任务状态存在可复现的竞态窗口。ComfyUI 进程误杀风险的代码修复已经完成，仍待实机复验。
...
### D-08（P0）移动网络 Tailscale 无法建立 Direct 连接
...
- [ ] **T-008 恢复移动网络 Direct 连接。** 复核手机端网络限制、运营商 NAT 和 UDP 打洞条件；在中国移动与中国广电 5G 下分别记录 `tailscale ping` 路径、延迟和稳定性。
