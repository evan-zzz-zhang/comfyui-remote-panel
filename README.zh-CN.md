...
- 权限不足或短暂 I/O 错误等不确定的文件系统检查结果不会被当成删除证据；只有在全部已登记输出都确认不存在时，才会移除 Job。
- 工作流启用/禁用状态在服务端公共执行路径统一校验，直接 API 和 Retry 也不能绕过；Configurator 草稿测试使用独立的显式测试入口。
- 存储统计会合并并去重旧文件表与 artifact 表；上传和保留素材复制会预留尚未写入的磁盘空间，准入时读取最新配额占用，落库时交接预留。取消会等待文件线程完成，清理未登记输入，并保留已归属任务的输入。
- SSE 心跳不会取消事件等待；断线重连后会对账已加载历史，不能把不完整的首屏快照误判为删除；视频流、SSE 等流式响应与普通响应使用同一套安全响应头。
